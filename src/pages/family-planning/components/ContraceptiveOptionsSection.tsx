...
import { contraceptiveOptions } from '@/mocks/familyPlanningData';

export default function ContraceptiveOptionsSection() {
        ...
        <div ref={ref} className={`transition-all duration-900 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="text-center mb-10 md:mb-14">
            <div className="inline-flex items-center px-3 py-1.5 rounded-full bg-accent-100/70 border border-accent-200/50 mb-4">
              <i className="ri-medicine-bottle-line text-accent-700 text-xs mr-1.5"></i>
              <span className="text-accent-700 text-xs font-semibold tracking-wide">Contraceptive Options</span>
            </div>
            ...
            <p className="text-foreground-600 text-sm md:text-base max-w-2xl mx-auto">
              Every woman is unique. Discover the full range of contraceptive methods and find what works best for your body, lifestyle, and future plans.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
            {contraceptiveOptions.map((option, index) => {
                ...
                >
                  ...
                  <div
                    className={`overflow-hidden transition-all duration-500 ease-out ${
                      isExpanded ? 'max-h-[700px] mt-5 opacity-100' : 'max-h-0 mt-0 opacity-0'
                    }`}
                  >
                    <div className="border-t border-background-200 pt-4 space-y-4">
                      ...
                      <div>
                        <h4 className="text-xs font-semibold text-foreground-500 uppercase tracking-wider mb-2">Key Benefits</h4>
                        <div className="flex flex-wrap gap-1.5">
                          {option.benefits.map((b) => (
                            <span key={b} className="px-2.5 py-1 bg-primary-50 text-primary-700 rounded-full text-xs">{b}</span>
                          ))}
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <h4 className="text-xs font-semibold text-foreground-500 uppercase tracking-wider mb-1">Duration</h4>
                          <p className="text-foreground-600 text-xs">{option.duration}</p>
                        </div>
                        <div>
                          <h4 className="text-xs font-semibold text-foreground-500 uppercase tracking-wider mb-1">Best For</h4>
                          <p className="text-foreground-600 text-xs">{option.suitability}</p>
                        </div>
                      </div>
                      <div>
                        <h4 className="text-xs font-semibold text-foreground-500 uppercase tracking-wider mb-2">Key Considerations</h4>
                        <p className="text-foreground-600 text-xs md:text-sm">{option.considerations}</p>
                      </div>
                    </div>
                  </div>
                </div>
