...
import { surgicalJourney } from '@/mocks/minimalInvasiveSurgeryData';

export default function SurgicalJourneySection() {
        ...
        <div ref={ref} className={`transition-all duration-900 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="text-center mb-10 md:mb-14">
            ...
            <p className="text-foreground-600 text-sm md:text-base max-w-2xl mx-auto">
              From your first consultation through complete recovery — here&apos;s what to expect at every stage of the surgical care pathway.
            </p>
          </div>

          <div className="relative">
            {/* Horizontal connecting line (desktop) */}
            <div className="hidden lg:block absolute top-8 left-[8%] right-[8%] h-0.5 bg-primary-200"></div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 md:gap-3">
              {surgicalJourney.map((step, index) => (
                <div
                  key={step.step}
