...
import { hysteroscopicComparisonTable } from '@/mocks/hysteroscopicSurgeryData';

export default function ProcedureComparisonSection() {
        ...
        <div ref={ref} className={`transition-all duration-900 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="text-center mb-10 md:mb-14">
            <div className="inline-flex items-center px-3 py-1.5 rounded-full bg-secondary-100/70 border border-secondary-200/50 mb-4">
              ...
              <span className="text-secondary-700 text-xs font-semibold tracking-wide">Procedure Comparison</span>
            </div>
            <h2 className="font-heading text-2xl md:text-3xl lg:text-4xl font-bold text-foreground-900 mb-3">
              Traditional Surgery vs Hysteroscopic Surgery
            </h2>
            <p className="text-foreground-600 text-sm md:text-base max-w-2xl mx-auto">
              Hysteroscopic surgery is the least invasive approach for treating uterine conditions — no incisions, no scars and same-day discharge.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-background-200/70 shadow-sm overflow-hidden">
            <div className="hidden md:grid grid-cols-12 gap-4 p-5 bg-primary-50 border-b border-primary-100">
              ...
              <div className="col-span-3">
                <span className="text-primary-600 text-xs font-semibold uppercase tracking-wider">Hysteroscopic Surgery</span>
              </div>
            ...
            <div className="divide-y divide-background-100">
              {hysteroscopicComparisonTable.map((row, index) => (
                <div
                  key={row.feature}
                ...
                >
                  ...
                  <div className="md:col-span-3 flex items-center">
                    <span className="text-primary-600 text-xs md:text-sm font-medium">{row.hysteroscopic}</span>
                  </div>
