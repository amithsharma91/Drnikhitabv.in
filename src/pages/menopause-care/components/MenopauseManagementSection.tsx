...
import { menopauseManagement } from '@/mocks/menopauseData';

export default function MenopauseManagementSection() {
        ...
        <div ref={ref} className={`transition-all duration-900 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="text-center mb-10 md:mb-14">
            <div className="inline-flex items-center px-3 py-1.5 rounded-full bg-accent-100/70 border border-accent-200/50 mb-4">
              <i className="ri-capsule-line text-accent-700 text-xs mr-1.5"></i>
              <span className="text-accent-700 text-xs font-semibold tracking-wide">Treatment & Support</span>
            </div>
            ...
            <p className="text-foreground-600 text-sm md:text-base max-w-2xl mx-auto">
              A comprehensive range of evidence-based treatment options tailored to your individual symptoms, preferences, and health profile.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
            {menopauseManagement.map((option, index) => (
              <div
                key={option.title}
              ...
              >
                <div className="w-12 h-12 rounded-xl bg-primary-100 flex items-center justify-center flex-shrink-0 group-hover:bg-primary-500 transition-colors duration-400">
                  <i className={`${option.icon} text-primary-600 group-hover:text-white text-xl transition-colors duration-400`}></i>
                </div>
                <div>
                  <h3 className="font-heading text-foreground-900 text-base font-semibold mb-2">{option.title}</h3>
                  <p className="text-foreground-500 text-xs md:text-sm leading-relaxed">{option.description}</p>
                </div>
              </div>
            ))}
          </div>
