...
interface VaccinationsCTAProps {
  onBookAppointment: () => void;
}

export default function VaccinationsCTA({ onBookAppointment }: VaccinationsCTAProps) {
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-4 md:px-6 lg:px-10 text-center">
        <div ref={ref} className={`transition-all duration-900 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="inline-flex items-center px-3 py-1.5 rounded-full bg-white/20 border border-white/30 mb-5">
            ...
            <span className="text-white text-xs font-semibold tracking-wide">Protect Your Health</span>
          </div>
          <h2 className="font-heading text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-4 leading-tight">
            Ready To Protect Your Future Health?
          </h2>
          <p className="text-white/90 text-sm md:text-base max-w-xl mx-auto mb-8 leading-relaxed">
            Schedule a vaccination consultation with Dr. Nikhita B Vadvadgi for personalised guidance on which vaccines are right for you at your current life stage.
          </p>
