...
import { fertilityAssessmentCards } from '@/mocks/fertilityCareData';

export default function FertilityAssessmentSection() {
        ...
        <div ref={ref} className={`transition-all duration-900 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="text-center mb-10 md:mb-14">
            <div className="inline-flex items-center px-3 py-1.5 rounded-full bg-accent-100/70 border border-accent-200/50 mb-4">
              <i className="ri-microscope-line text-accent-700 text-xs mr-1.5"></i>
              <span className="text-accent-700 text-xs font-semibold tracking-wide">Fertility Assessment</span>
            </div>
            ...
            <p className="text-foreground-600 text-sm md:text-base max-w-2xl mx-auto">
              A thorough assessment covering every aspect that influences your fertility. Tap any card to explore in detail.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
            {fertilityAssessmentCards.map((card, index) => {
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-2xl p-5 md:p-6 cursor-pointer transition-all duration-500 ease-out border hover:shadow-lg hover:-translate-y-1 ${
                    isExpanded
                      ? 'border-primary-300 shadow-md ring-1 ring-primary-200 lg:col-span-2 lg:row-span-2'
                ...
                >
                  ...
                  <div
                    className={`overflow-hidden transition-all duration-500 ease-out ${
                      isExpanded ? 'max-h-[800px] mt-5 opacity-100' : 'max-h-0 mt-0 opacity-0'
                    }`}
                  >
                    <div className="border-t border-background-200 pt-4 space-y-4">
                      ...
                      <p className="text-foreground-600 text-xs md:text-sm leading-relaxed">{card.details}</p>
                      <div>
                        <h4 className="text-xs font-semibold text-foreground-500 uppercase tracking-wider mb-2">Common Signs</h4>
                      ...
                      <div>
                        <h4 className="text-xs font-semibold text-foreground-500 uppercase tracking-wider mb-2">Evaluation Approach</h4>
                        <p className="text-foreground-600 text-xs md:text-sm">{card.evaluation}</p>
                      </div>
                    </div>
                  </div>
                </div>
